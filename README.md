...
## 9. Cross-fidelity aerodynamic comparison

The validated Euler solution provides the inviscid reference for the Mach 2.5 diamond airfoil.

The next stage is to determine what changes when viscosity, wall shear stress and boundary-layer development are introduced through the Spalart-Allmaras RANS model.

The comparison is made at the same reference condition:
...
Reynolds number      ≈ 5.82 × 10^7
```

The purpose is not simply to compare two values of `Cd`.
...
- boundary-layer response;
- shock and expansion structure.

> **The Euler solution defines the inviscid aerodynamic reference. The RANS solution shows how that reference changes when viscous physics are introduced.**

### 9.1 Inviscid reference

The final WENO5-HLLC Euler solution gives:
...
The corresponding analytical predictions are:

| Method | `Cd` | `Cl` |
|---|---:|---:|
| Ackeret theory | 0.030752 | 0.152345 |
| Nonlinear shock-expansion theory | 0.030572 | 0.150450 |
| WENO5-HLLC Euler | 0.031606 | 0.156778 |

Relative to Ackeret theory, the Euler solution differs by approximately:
...
The Euler result therefore provides the reference aerodynamic state for the viscous comparison.

### 9.2 Current SA-RANS state

The SA-RANS simulation has not yet been accepted as the final viscous reference.

At iteration `8000`, the current force coefficients are:
...
The drag decomposition is:

```text
Cd
│
├── pressure drag  = 0.027900
│
└── viscous drag   = 0.006378

total Cd           = 0.034278
```

At this stage, approximately:

```text
81.4 % of total drag → pressure contribution
18.6 % of total drag → viscous contribution
```

These values remain provisional until the complete RANS convergence requirement is satisfied.

### 9.3 Euler and RANS force comparison

Using the current iteration `8000` RANS state:

| Quantity | Euler | SA-RANS | Change |
|---|---:|---:|---:|
| `Cd` | 0.031606 | 0.034278 | +8.45% |
| `Cl` | 0.156778 | 0.134790 | -14.02% |
| `L/D` | 4.960 | 3.932 | -20.73% |

The current viscous solution therefore shows:

- higher total drag;
- lower lift;
- lower lift-to-drag ratio;
- a finite wall-shear contribution;
- a change in the pressure contribution itself.

The last point is particularly important.

The RANS pressure drag is:

```text
Cd,p = 0.027900
```

while the Euler drag is:

```text
Cd = 0.031606
```

Since the Euler solution contains no viscous wall stress, its drag is entirely pressure generated.

The current RANS pressure contribution is therefore approximately `11.73%` lower than the Euler value.

```text
Euler pressure loading
        ↓
viscous boundary layer introduced
        ↓
effective outer-flow behaviour changes
        ↓
pressure loading changes
        +
wall shear appears
        ↓
new total aerodynamic force
```

The RANS result is therefore not simply:

```text
Euler drag + skin friction
```

The viscous solution modifies both the pressure field and the wall-shear contribution.

### 9.4 Physical interpretation

For the sharp diamond geometry,
...
giving approximate leading-edge compression turns of:

```text
upper surface ≈ 0.71 deg
lower surface ≈ 10.71 deg
```

The corresponding analytical weak-shock angles are approximately:

```text
upper shock angle ≈ 24.09 deg
lower shock angle ≈ 32.53 deg
```

The lower surface therefore experiences the stronger leading-edge compression.

This produces the asymmetric pressure field responsible for the positive lift of the reference configuration.

The Euler solution captures this through the inviscid shock-expansion system.

The RANS solution introduces additional physics:

- wall shear stress;
- boundary-layer growth;
- displacement effects;
- viscous interaction with the external flow;
- changes in local pressure recovery;
- changes in integrated pressure loading.

The aerodynamic chain becomes:
...
This is the physical connection that will be examined when the final RANS flow field is post-processed.

### 9.5 Pressure and viscous drag balance

The current Euler-to-RANS drag change can be separated directly.
...
The pressure-drag change relative to Euler is:

```text
ΔCd,p = -0.003706
```

while the new viscous contribution is:

```text
Cd,v = +0.006378
```

giving the final difference:
...
The current behaviour can therefore be summarised as:

```text
pressure drag decreases
          +
viscous drag appears
          ↓
viscous penalty is larger
than pressure-drag reduction
          ↓
total drag increases
```

This decomposition is more informative than comparing total `Cd` alone.

### 9.6 Aerodynamic efficiency

The Euler solution gives:

```text
L/D = 4.960
```

The current SA-RANS state gives:
...
The reduction occurs because both sides of the aerodynamic balance move unfavourably:
...
The current RANS calculation therefore predicts a substantial viscous effect on overall aerodynamic efficiency.

The final magnitude will only be accepted after the RANS solution has fully converged.

### 9.7 RANS convergence status

The RANS force history must be examined component by component.
...
```text
pressure drag change ≈ -0.04 %
viscous drag change  ≈ -3.16 %
total drag change    ≈ -0.63 %
```

The pressure component is already extremely stable.

The viscous contribution is still reducing.

This means that apparent stability of the total `Cd` is not sufficient to declare the solution converged.

```text
total Cd appears nearly stable
             ↓
separate force components checked
             ↓
pressure drag nearly stationary
             ↓
viscous drag still evolving
             ↓
RANS solution not yet final
```

> **The SA-RANS reference will not be accepted until pressure drag, viscous drag, lift and moment are simultaneously stationary.**

### 9.8 Quantities required from the final RANS solution

Once the RANS reference has converged, the cross-fidelity comparison will include:

- final `Cl`, `Cd` and `Cm`;
- pressure and viscous drag decomposition;
- `L/D`;
- surface `Cp`;
...
- Mach-number field;
- pressure field;
- leading-edge shock structure;
- mid-chord expansion structure;
- rear-panel pressure recovery.

The final comparison will therefore connect the integrated forces to the flow physics responsible for them.

```text
Euler reference
      ↓
shock-expansion physics
      ↓
SA-RANS reference
      ↓
boundary-layer + viscous interaction
      ↓
difference in Cp and wall shear
      ↓
difference in Cl, Cd and Cm
```

### 9.9 Acceptance of the viscous reference

The final SA-RANS solution will become the production reference only after:

- force coefficients have converged;
- pressure and viscous drag components are stationary;
- the moment coefficient is stable;
- wall quantities show physically consistent behaviour;
- the shock and expansion system is resolved correctly;
- the solution passes the domain-independence study.

Only then will the CFD setup be frozen for the controlled angle-of-attack sweep and geometry optimisation described in Section 8.

The required sequence is:

```text
validated Euler reference
          ↓
converged SA-RANS reference
          ↓
cross-fidelity comparison
          ↓
domain independence
          ↓
freeze production methodology
          ↓
AoA sweep
          ↓
...
> **The purpose of the cross-fidelity study is not to decide which solver gives the better-looking result. It is to identify which aerodynamic behaviour changes when additional physics are introduced, and why.**
